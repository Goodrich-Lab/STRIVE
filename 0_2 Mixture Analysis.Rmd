...
# Mixture analysis using AST_ALT endpoint
```{r}
...
plot_pfas <- function(res, title) {
  ggplot(res, aes(x = coef, y = fct_reorder(type, coef), color = sig)) +
...
plot_legacy <- plot_pfas(res_legacy, "Effect of Legacy PFAS on AST/ALT Ratio")

# Plot results for emerging PFAS
plot_emerging <- plot_pfas(res_emerging, "Effect of Emerging PFAS on AST/ALT Ratio")
...
(panel_a <- ggplot(
  ...
  ggtitle("Overall Mixture Effect on AST/ALT using qgcomp") +
