...
data_min <- min(result_df_filtered)
data_max <- max(result_df_filtered)

breaks <- sort(unique(c(seq(min(result_df_filtered), 0 - 1e-5, length.out = 50), 0, seq(0 + 1e-5, max(result_df_filtered), length.out = 51))))

pheatmap(result_df_filtered, 
         cluster_rows = TRUE, 
         cluster_cols = TRUE, 
         color = custom_colors,  # Use custom color palette
         breaks = breaks,  # Set breaks to include the midpoint
...
data_min <- min(first_set_df)
data_max <- max(first_set_df)

breaks <- sort(unique(c(seq(min(first_set_df), 0 - 1e-5, length.out = 50), 0, seq(0 + 1e-5, max(first_set_df), length.out = 51))))
...
data_min <- min(second_set_df)
data_max <- max(second_set_df)

breaks <- sort(unique(c(seq(min(second_set_df), 0 - 1e-5, length.out = 50), 0, seq(0 + 1e-5, max(second_set_df), length.out = 51))))
...
#excluding source for releveling
cate_conf <-setdiff(cate_conf, "source")
...
###
# for source
generate_covariate_name <- function(base_name, level) {
  return(paste0(base_name, "_", gsub(" ", "_", level)))
}

fit_all_models <- function(pfas_types, base_covariate, data) {
  all_results <- list()
  
  # Ensure base_covariate is an unordered factor
  data[[base_covariate]] <- as.factor(data[[base_covariate]])
  levels_source <- levels(data[[base_covariate]])
  all_levels <- levels_source  
  
  for (pfas in pfas_types) {
    for (level in all_levels) {
      
      covariate_name <- generate_covariate_name(base_covariate, level)
      
      # Relevel the factor
      data[[covariate_name]] <- relevel(data[[base_covariate]], ref = level)
      
      model_results <- fit_model_cat(pfas, covariate_name, data)
      model_results$Reference_Level <- level
      model_results$Covariate_Name <- covariate_name
      all_results[[paste(pfas, level, sep = "_")]] <- model_results
    }
  }
  
  final_results_df <- do.call(rbind, all_results)
  final_results_df <- final_results_df[, !(names(final_results_df) %in% c("Reference_Level","Covariate_Name"))]
  return(final_results_df)
}

source_results <- fit_all_models(pfas_name_scld, "source", data)

print(source_results)

categorical_results <- rbind(categorical_results, source_results)
###
all_results <- rbind(continuous_results, categorical_results)
...
custom_colors <- colorRampPalette(brewer.pal(3, "YlOrRd"))(100)
...
grid.text("R squared", x = 0.96, y = 0.90, gp = gpar(fontsize = 10))
grid.text("*adjusted P<0.05", x = 0.96, y = 0.86, gp = gpar(fontsize = 6, fontface = "italic"))

#####
# Forest plot for the correlation coefficient (SE)
sig_var <- c('age_at_enrollment','sq_drink_alcohol','sq_average_drink_per_day', 'sex','bmi','smoking','sq_water_bottled','sq_water_other_type', 'sq_water_faucet_filter','source_UNC','source_DUKE','source_Emory','source_NCSU')
...
new_sig_factor <- c(
  "sq_water_bottledYes" = "Water bottled, Yes vs No",
  "sq_water_other_typeYes" ="Water other type, Yes vs No",
  "sq_water_faucet_filterYes" = "Water faucet filtered, Yes vs No",
  "sq_drink_alcoholYes, current drinker" = "Current drinker vs Nondrinker",
  "sq_drink_alcoholNo, former drinker (stopped)" = "Past drinker vs Nondrinker",
  ...
  "source_UNCNCSU" = "NCSU vs UNC",
  "source_UNCEmory" = "Emory vs UNC",
  "source_UNCDUKE" ="Duke vs UNC",
  "source_NCSUUNC" ="UNC vs NCSU",
  "source_NCSUEmory" = "Emory vs NCSU",
  "source_NCSUDUKE" ="Duke vs NCSU",
  "source_EmoryUNC" = "UNC vs Emory",
  "source_EmoryNCSU" = "NCSU vs Emory",
  "source_EmoryDUKE" = "Duke vs Emory",
  "source_DUKEUNC" = "UNC vs Duke",
  "source_DUKENCSU" ="NCSU vs Duke",
  "source_DUKEEmory" = "Emory vs Duke"
)
...
forest_plot <- ggplot(combined_mean, aes(x = Average_Coeff, y = Factor, color = Average_Coeff)) +
  ...
  scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
                       limit = c(min(combined_mean$Average_Coeff), max(combined_mean$Average_Coeff)), 
                       space = "Lab", 
                       name="Coefficient") +
...
demo <-for_p_filtered %>% filter(Confounders %in% c('age_at_enrollment','bmi','sex'))
alcohol_smoke <-for_p_filtered %>% filter(Confounders %in% c('smoking','sq_drink_alcohol','sq_average_drink_per_day'))
water <-for_p_filtered %>% filter(Confounders %in% c("sq_water_faucet_filter","sq_water_bottled", "sq_water_other_type"))
site <- for_p_filtered %>% filter(Confounders %in% c("source_Emory","source_DUKE","source_NCSU","source_UNC"))

demo$PFAS <- factor(demo$PFAS, levels = unique(demo$PFAS))

forest_plot <- ggplot(demo, aes(x = Coeff, y = PFAS, color = Coeff)) +
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) +
  scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
                       limit = c(min(for_p_filtered$Coeff), max(for_p_filtered$Coeff)), 
                       space = "Lab", 
                       name = "Coefficient") +
  labs(
    x = "Coefficient",
    y = "PFAS",
    color = "Coefficient",
    title = "Coefficients of demographic factors by individual PFAS"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
  ...
  facet_grid(PFAS_Group ~ Factor, scales = "free_y", space = "free_y", drop = TRUE)

print(forest_plot)

forest_plot1 <- ggplot(alcohol_smoke, aes(x = Coeff, y = PFAS, color = Coeff)) +
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) +
  scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
  ...
  labs(
    ...
    y = "PFAS",
    color = "Coefficient",
    title = "Coefficients of alcohol drinking and smoking factors by individual PFAS"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
forest_plot2 <- ggplot(water, aes(x = Coeff, y = PFAS, color = Coeff)) +
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) +
  scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
  ...
  labs(
    ...
    y = "PFAS",
    color = "Coefficient",
    title = "Coefficients of water factors by individual PFAS"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
forest_plot3 <- ggplot(site, aes(x = Coeff, y = PFAS, color = Coeff)) +
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) +
  scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
  ...
  labs(
    ...
    y = "PFAS",
    color = "Coefficient",
    title = "Coefficients of all possible comparison between sites by individual PFAS"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
forest_plot <- ggplot(new_for_p_cont, aes(x = Coeff, y = Factor, color = Coeff)) +
  ...
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) +
   scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 0, 
                       limit = c(min(new_for_p_cont$Coeff), max(new_for_p_cont$Coeff)), 
                       space = "Lab", 
                       name = "Coefficient") +
  labs(
    x = "Coefficients",
...
forest_plot <- ggplot(new_for_p_cat, aes(x = OR, y = Factor, color = OR)) +
  ...
  geom_errorbarh(aes(xmin = OR_CI_Lower, xmax = OR_CI_Upper), height = 0.2) +
    scale_color_gradient2(low = "blue", high = "red", mid = "grey", 
                       midpoint = 1, 
                       limit = c(min(new_for_p_cat$OR), max(new_for_p_cat$OR)), 
                       space = "Lab", 
                       name = "OR") +
  labs(
    x = "Odds Ratios",
...
#Table 1
```{r}
retemp_data <- data %>% dplyr::filter(!is.na(alt_cat2))
stats <- table1(~. |retemp_data$alt_cat2, data = retemp_data %>% dplyr::select(all_of(pfas_name_analysis), all_of(potential_conf),c("alt_u_l","ast_u_l")))
